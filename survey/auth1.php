...
$surveyId = isset($_REQUEST['surveyid'])? $_REQUEST['surveyid'] : 1;

$userId = Register($openid);

//pass sessionId to the start page.
$sessionId = CreateNewSession($userId, $ownerId, $channelId, $surveyId);
...
function CreateNewSession($userId, $ownerId, $channelId, $surveyId)
{
	global $db;
	...
	array($surveyId, $time, $userId, $ownerId, $channelId));

	return $db->Insert_ID();
}

$redir = $urlRoot."survey_question1.php?sid=".$sessionId."&uid=".$openid."&cid=".$channelId;
